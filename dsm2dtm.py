...
def replace_values(
    rasterA_path, rasterB_path, out_path, no_data_value=-99999.0, threshold=5.0
):
    """
...
def get_res_and_downsample(dsm_path, temp_dir, resolution=0.3):
    ...
    if dsm_crs != 4326:
        if x_res < resolution or y_res < resolution:
            target_res = resolution  # downsample to this resolution (in meters)
...
def get_updated_params(dsm_path, search_radius, smoothen_radius, resolution=0.3):
    ...
    if dsm_crs != 4326:
        if x_res > resolution or y_res > resolution:
            search_radius = int((min(x_res, y_res) * search_radius) / resolution)
            smoothen_radius = int((min(x_res, y_res) * smoothen_radius) / resolution)
    else:
        if x_res > 2.514e-06 or y_res > 2.514e-06:
...
def main(
    ...
    dsm_path = get_res_and_downsample(dsm_path, temp_dir, target_res)
    # get updated params wrt to DSM resolution
    search_radius, smoothen_radius = get_updated_params(
        dsm_path, search_radius, smoothen_radius, target_res
    )
...
if __name__ == "__main__":
    ...
    out_dir = 'chua_kham_son_out'
    target_res = 0.3
    dtm_path = main(dsm_path=dsm_path, out_dir=out_dir, target_res=target_res)
